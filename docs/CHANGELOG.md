...
- **기술 스택 "현재 vs 추후" 명시** — 데모 단계 구현(SQLite→정적 JSON→vanilla JS)이 확정·동작 중이며 **추후 변경 가능**(FastAPI 등)임을 문서에 명시
  ...
  - `README.md` 기술 스택 표를 "목표(지향) 아키텍처"로 명시하고 현재 데모 구현과 구분
- **`player_comparison_spec.md`를 `CLAUDE.md` 문서표·업데이트 규칙에 등록** (기존 미등록)
- **`todo_list.md` #0**: "스택 정본 확정 필요" 과제 → "방향 결정됨(데모=SQLite/JSON), 추적 종료"로 전환

### Changed (2026-06-07) — 비교 시안 v4.3 (선수 카드 정보·최대 4명)
- **선수 카드 정보 재배치** — 포지션 옆에 **생년월일** 표기(`FW · 1999-12-12`), 출전시간·경기수는 SEASON 하위 `출전` 행으로 이동(v2 카드). v1 헤더도 동일 정보 반영
- **생년월일 추출 추가** — `export_comparison_data.py`가 `player_master.birth_date`를 JSON에 포함(`birth`)
- **최대 선택 선수 3명 → 4명** — `PCOLORS`에 4번째 색(`#A45CD6`) 추가, 카드/히트맵/swarm·모달·추가슬롯 모두 4명 대응
- **카드 그리드 4열 고정** — 기존 `auto-fit+1fr`이 카드 3장을 폭에 맞춰 늘려 4번째(추가 슬롯)가 다음 줄로 밀리던 문제 → `repeat(4,minmax(0,1fr))`로 같은 행 우측 배치(모바일 `max-width:900px` 2열)
- **카드 등높이/등너비** — 카드 높이 미세 차이(★ 들어간 행의 line-height 누적) 해결: 그리드 `align-items:stretch`+`.card height:100%`, 스탯 행 `line-height:18px`·별표 `line-height:1`로 행 높이 고정
- **드롭다운 "상위 8개" 버튼 제거** — 세부 스탯 드롭다운 도구에서 일괄선택 제외, `전체 해제`만 유지(1·2안)
- **`start_demo.bat` 추가** — 더블클릭으로 프로젝트 루트 정적 서버(8770) 실행 + 브라우저 자동 오픈(로컬 데모 실행 편의)

### Fixed (2026-06-07) — 비교 시안 데이터 정합성 (v4.2)
- **파생 비율 표시/순위 분리** — 비율(rate)을 합계·90분당 모드에선 `den>0`이면 **항상 표시**(`_rateShow`), 백분위 순위에서만 `minDen` 가드(`_rate`). 기존엔 minDen 미달 시 모든 모드에서 빈값이라 다수(예: 유효슈팅률 77%·드리블 81%)가 사라지던 문제 해소. 문선민 2026(패스 48/35)→ 72.9% 정상 표시
- **출전 이상치(master_id 충돌) 임시 드롭** — `export_comparison_data.py`에서 `games > 리그 라운드상한+2`(KL1>40/KL2>43)인 **11건 제외**(이동경·박민수 등 동명이인 병합). 경계 2건(2025 KL1 39·40경기, 승강PO 가능)은 보존+검토 로그. 결과 1,513→**1,502 선수-시즌**
  - 근본 수정(동명이인 master 분리·재매핑)은 `todo_list.md #11`로 병행

### Changed (2026-06-07) — 비교 시안 v4.1 (카드 출전시간·swarm 리디자인)
- **선수 카드/헤더에 출전시간 표기** — 포지션 옆 `FW · 2,493분 (29경기)` (v1 헤더·v2 카드 공통)
- **swarm 백분위 차트 리디자인 (2안)**:
  - 점 위 텍스트 핀 제거 → **차트 카드 헤더 우측에 컬러 readout**(`선수명 백분위 N`)으로 분리(시각화 가림 해소). `%ile` 표기 폐기
  - **배경 점 반투명**(rgba α≈0.38)으로 겹침 보완 + 점 크기 미세 변주, 0/25/50/75/100% 점선 격자
  - 선택 선수는 **컬러 링**만 표시(텍스트 없음), 동일 x 충돌 시 수직 분리
  - **차트=카드 단위**로 분리해 차트 간 여백(`margin-bottom`) 확보

### Added (2026-06-07) — 비교 시안 실데이터 연결 (v4.0)
- **`scripts/export_comparison_data.py` 신규** — kleague.db `player_match_stats`를 (시즌·리그·선수) 단위로 집계해 `demo html/players.generated.json` 생성. 대회명 정규화(`하나은행 K리그1`/`K리그2 2025`→`K리그1/2`, 슈퍼컵 제외), 팀=최다출전팀, 포지션 매핑(TM→FW/MF/DF/GK)
  - 추출 결과: **1,513 선수-시즌** (2024 KL1 443 / 2024 KL2 349 / 2025 KL1 417 / 2026 KL1 304), 63 measure(52 count + 11 rate), 5 카테고리, 2.0MB
- **measure 5개 카테고리 확정** — 슈팅(7+3) / 패스(24+4) / 수비(10+3) / 드리블·세트피스(6+1) / 규율·기타(5). EDA로 미분류 11개 식별 후 2개 카테고리 신설
- **파생 비율 measure 11종** — 유효슈팅률·득점전환율·PA내비중·패스/롱/크로스/전방 성공률·태클/지상/공중 승률·드리블 성공률. `Σ성공/Σ시도`로 집계, 최소 시도 가드(`minDen`)
- **`comparison-data.js` v2.0** — 합성 생성기 폐기, JSON 로더 + 실데이터 API(`val`/`percentile`/`fmtVal`/`swarmVal`). 단위 처리: count=합계/90분당/백분위(per90 기준·최소 450분 필터), rate=비율값(합계·90분당 동일)/비율 순위
- **v1·v2 HTML v4.0** — 카테고리 버튼 동적 생성(5개), 2레벨 드롭다운 전체 measure 노출(스크롤), **최대 8개 선택 캡(기본 6)**, 비율/결측(`–`) 처리, 비동기 부팅. **2안 swarm은 실제 리그 분포(per90/비율 축, beeswarm)** 위에 선택 선수 백분위 하이라이트
- **데이터 정합성**: 추출 시 (master,match) 중복 행 dedup 처리. EDA로 **master_id 충돌(동명이인 병합)로 시즌 출전수>42인 11건** 검출 → `todo_list.md #11` 등록(상위 ETL 매핑 이슈, 추출은 경고만 출력)
