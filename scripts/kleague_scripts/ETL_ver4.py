...
import sys, sqlite3, pandas as pd, os
...
def import_csv_to_db(csv_path):
    ...
    # 홈여부: 경기명의 (H)/(A)에서 파생. (H)=현재 행의 팀이 홈=1, (A)=어웨이=0
    if "경기명" in df.columns:
        df["홈여부"] = df["경기명"].str.strip().str.contains(r'\(H\)$').astype(int)
    elif "홈여부" not in df.columns:
        df["홈여부"] = 1

    # 상대팀명: 앞뒤 공백 + (H)/(A) suffix 제거
    if "상대팀명" in df.columns:
        df["상대팀명"] = df["상대팀명"].astype(str).str.strip().str.replace(r'\s*\([HA]\)$', '', regex=True).str.strip()

    # 라운드: 앞뒤 공백 제거
    if "라운드" in df.columns:
        df["라운드"] = df["라운드"].astype(str).str.strip()
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            is_home = bool(row.get("홈여부", 1))
            home_id = team_id if is_home else opponent_team_id
            away_id = opponent_team_id if is_home else team_id
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?)
                """,
                (competition_id, round_number, home_id, away_id),
            )
            ...
            cursor.execute(
                ...
                (player_name, position, back_number, team_name, team_id)
                VALUES (?, ?, ?, ?, ?)
                """,
                (
                    player_name,
            ...
            cursor.execute(
                "SELECT player_id FROM players WHERE player_name=? AND back_number=?",
                (player_name, back_number),
            )
...
print("insert_dataframe 존재 여부:",
      "insert_dataframe" in globals())
